...
- **Manual log-entry dialog: migrate to the new label selector.** The Create / Edit log-entry dialog still uses the old `Adw.ComboRow` label picker. Convert to the same `Adw.ExpanderRow` (show-enable-switch) + chooser sub-row pattern used on the timer Setup and Done pages, so the affordance is consistent everywhere a session gets a label. Reuse `MeditateWindow::push_label_chooser` — no new chooser code needed; just swap the row template and rewire the persistence calls.
