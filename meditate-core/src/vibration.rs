...
// Preview state machine lives in `crate::preview` so the sound
// chooser can share it; re-exported below for back-compat with
// callers that already reach for `meditate_core::vibration::*`.
...
mod tests {
    ...
    // PreviewToggle behaviour tests live in `crate::preview::tests`
    // alongside the type itself; vibration's test surface focuses
    // on the math below.
